...
class GaussianDiffusion:
    ...
    def sample(self, text_emb, text_mask, shape):
        ...
        for i in reversed(range(0, self.timesteps)):
            ...
            mean = (1 / torch.sqrt(alpha)) * (img - (beta / torch.sqrt(1 - alpha_bar)) * noise_pred)
            if i > 0:
    ...
    def ddim_sample(
        ...
        if cfg_scale != 1.0:
            raise NotImplementedError(
                "CFG is not supported because the model was not trained with condition dropout. "
                "Please use cfg_scale=1.0."
            )
        ...
        b = shape[0]
        img = init_noise if init_noise is not None else torch.randn(shape, device=self.device)
        ...
        for i in reversed(range(len(time_steps))):
            ...
            pred_x0 = (
                img - torch.sqrt(torch.clamp(1.0 - alpha_bar_cur, min=0.0)) * noise_pred
            ...
            sigma = eta * torch.sqrt(
                torch.clamp(
                    ...
                    * (1.0 - alpha_bar_cur / torch.clamp(alpha_bar_prev, min=1e-8)),
                    min=0.0,
                )
            )
            ...
            if not torch.isfinite(img).all():
                raise FloatingPointError(f"Non-finite sample detected at DDIM step {t_cur}")
